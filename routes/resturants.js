...
router.get(`/`, async (req, res) => {
    try {
        const resturantList = await Resturant.find();
        res.status(200).send(resturantList);
      } catch (error) {
...
router.delete('/:id', (req, res) => {
    Resturant.findByIdAndRemove(req.params.id)
        .then((resturant) => {
            if (resturant) {
                return res.status(200).json({
                    success: true,
                    message: 'the resturant is deleted!',
                });
            } else {
                return res
                    .status(404)
...
router.get(`/get/count`, async (req, res) => {
    const productCount = await Resturant.countDocuments((count) => count);
